...
run_weather() {
  __process_settings
  local tmp_file="${TMUX_DIR_TEMPORARY}/weather_yahoo.txt"
  local weather
...
__yahoo_weather() {
  ...
  if [ -z "$degree" ]; then
    ...
    if [ "$?" -eq "0" ]; then
      ...
      # echo $(echo "$weather_data" | "$gnugrep" "yweather:astronomy" | sed 's/^\(.*\)sunset.*/\1/' | sed 's/^.*sunrise="\(.*m\)".*/\1/')
      sunrise=$(date -v "$(echo "$weather_data" | "$gnugrep" "yweather:astronomy" | sed 's/^\(.*\)sunset.*/\1/' | sed 's/^.*sunrise="\(.*m\)".*/\1/')" +%H%M)
      sunset=$(date -v "$(echo "$weather_data" | "$gnugrep" "yweather:astronomy" | sed 's/^.*sunset="\(.*m\)".*/\1/')" +%H%M)
